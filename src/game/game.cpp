...
std::string Game::generateHighscoreQueryForEntries(const std::string &categoryName, const std::string &worldName, uint32_t page, uint8_t entriesPerPage, uint32_t vocation) {
	...
	query << "SELECT *, @row AS `entries`, " << page << " AS `page` FROM ("
		  << "SELECT *, (@row := @row + 1) AS `rn` FROM ("
		  << "SELECT `p`.`id`, `p`.`name`, `p`.`level`, `p`.`vocation`, `w`.`name` AS `worldName`, "
		  << pointsExpression << " AS `points`, "
		  << "@curRank := IF(@prevRank = " << pointsExpression << ", @curRank, IF(@prevRank := " << pointsExpression << ", @curRank + 1, @curRank + 1)) AS `rank` "
		  << fromClause << " INNER JOIN `worlds` `w` ON `p`.`world_id` = `w`.`id`, "
		  << "(SELECT @curRank := 0, @prevRank := NULL, @row := 0) `r` "
		  << "WHERE `group_id` < " << static_cast<int>(GROUP_TYPE_GAMEMASTER);
	...
	if (!worldName.empty()) {
		query << " AND `w`.`name` = " << Database::getInstance().escapeString(worldName);
	}

	query << " ORDER BY " << pointsExpression << " DESC) `t`) `T`"
...
std::string Game::generateHighscoreQueryForOurRank(const std::string &categoryName, uint8_t entriesPerPage, uint32_t playerGUID, uint32_t vocation, const std::string &selectedWorld) {
	...
	query << "SELECT *, @row AS `entries`, (@ourRow DIV " << entriesStr << ") + 1 AS `page` FROM ("
		  << "SELECT *, (@row := @row + 1) AS `rn`, @ourRow := IF(`id` = " << playerGUID << ", @row - 1, @ourRow) AS `rw` FROM ("
		  << "SELECT `p`.`id`, `p`.`name`, `p`.`level`, `p`.`vocation`, `w`.`name` AS `worldName`, "
		  << pointsExpression << " AS `points`, "
		  << "@curRank := IF(@prevRank = " << pointsExpression << ", @curRank, IF(@prevRank := " << pointsExpression << ", @curRank + 1, @curRank + 1)) AS `rank` "
		  << fromClause << " INNER JOIN `worlds` `w` ON `p`.`world_id` = `w`.`id`, "
	...
	if (!selectedWorld.empty()) {
		query << " AND `w`.`name` = " << Database::getInstance().escapeString(selectedWorld);
	}

	query << " ORDER BY " << pointsExpression << " DESC) `t`) `T`"
		  << " WHERE `rn` > ((@ourRow DIV " << entriesStr << ") * " << entriesStr << ")"
		  << " AND `rn` <= (((@ourRow DIV " << entriesStr << ") * " << entriesStr << ") + " << entriesStr << ")";
